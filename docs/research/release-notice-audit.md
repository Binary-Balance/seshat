...
dependency, and notice evidence for issue #6. It does not record publication,
merge approval, or legal clearance.
...
Before public npm or GitHub publication, record the release-owner full-use
Visual Studio license basis and the Section 5 downstream-terms decision. This
is a publication condition, not an additional package-file or notice change in
the current audit.

If source inputs, the package payload, linked runtime, or notice changes,
refresh the corresponding hashes and this scoped review. Publication and merge
approval remain outside this audit.
