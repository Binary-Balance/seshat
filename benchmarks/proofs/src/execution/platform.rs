...
mod windows {
    use super::*;
    use std::{
        ffi::c_void, mem::size_of, os::windows::io::AsRawHandle, ptr::null_mut, sync::OnceLock,
    };
    use windows_sys::Win32::{
        Foundation::{
            CloseHandle, ERROR_NO_MORE_FILES, FALSE, HANDLE, INVALID_HANDLE_VALUE, TRUE,
    ...
    impl Job {
        ...
        fn stop(&self) -> Result<(), String> {
            // A naturally exited leader leaves a signaled job. Avoid turning that normal state
            // into an error while still waiting for descendants after a hard termination.
            ...
            let state = unsafe { WaitForSingleObject(self.0, 5_000) };
            match state {
                WAIT_OBJECT_0 => Ok(()),
                WAIT_FAILED => Err(format!(
                    "wait for Windows job cleanup: {}",
                    io::Error::last_os_error()
                )),
                WAIT_TIMEOUT => Err("wait for Windows job cleanup timed out".into()),
                _ => Err(format!("wait for Windows job cleanup returned {state}")),
            }
        }
    }
        ...
        fn cleanup(&mut self) -> Result<(), String> {
            ...
            if self.assigned {
                ...
                if unsafe { TerminateProcess(child.as_raw_handle() as HANDLE, 1) } == FALSE {
                    ...
                    let _ = child.kill();
                    let _ = child.wait();
                    return Err(format!("terminate suspended child: {error}"));
                }
            }
            child
                .wait()
...
impl ManagedChild {
    ...
    pub(super) fn force_cleanup(&mut self) -> Result<(), String> {
        let stop = self.stop_tree();
        if stop.is_err() {
            let _ = self.kill_leader();
        }
        let wait = self.wait().map(|_| ()).map_err(|e| e.to_string());
        stop.and(wait)
    }
}
