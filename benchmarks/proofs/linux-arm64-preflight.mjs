...
function collect() {
  ...
  return {
    schemaVersion: 1,
    kind: 'seshat-linux-arm64-preflight',
...
function validate(report) {
  ...
  if (environment.platform !== 'linux') {
    failures.push(`OS: expected Linux, got ${environment.platform ?? 'unknown'}`);
  }
  ...
  if (environment.glibc !== expected.glibc) {
    failures.push(`glibc: expected ${expected.glibc}, got ${environment.glibc ?? 'unknown'}`);
  }
  if (environment.node.version !== expected.node) {
    failures.push(`Node: expected ${expected.node}, got ${environment.node.version ?? 'unknown'}`);
  }
  if (!environment.kill.available) {
    failures.push('kill: POSIX kill -0 is unavailable');
  }
  const rust = environment.toolchain.rust;
  if (rust.rustc.available && rust.host !== expected.rustHost) {
...
function selfCheck() {
  const passing = {
    environment: {
      ...
      architecture: {node: 'arm64', unameMachine: 'aarch64'},
      glibc: '2.35',
  ...
  for (const [change, expectedMessage] of [
    ...
    [report => { report.environment.architecture.node = 'x64'; }, 'process.arch'],
    [report => { report.environment.architecture.unameMachine = 'x86_64'; }, 'uname -m'],
    ...
    [report => { report.environment.toolchain.rust.rustc.available = true; report.environment.toolchain.rust.host = 'x86_64-unknown-linux-gnu'; }, 'Rust host'],
  ]) {
    const report = structuredClone(passing);
    change(report);
    assert.match(validate(report).join('\n'), new RegExp(expectedMessage.replace(/[().]/g, '\\$&')));
  }
  console.log('Linux ARM64 preflight self-check passed');
}

function outputPath() {
  const index = process.argv.indexOf('--output');
  if (index < 0) return 'linux-arm64-preflight.json';
...
if (process.argv.includes('--self-check')) {
  ...
  if (failures.length) {
    ...
    console.log(`Linux ARM64 preflight passed. Artifact: ${path}`);
  }
}
