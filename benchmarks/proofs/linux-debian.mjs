...
assert.equal(process.argv.length,4,'usage: node benchmarks/proofs/linux-debian.mjs <archive directory> <pack result.json>');
...
const result={os,kernel:release(),architecture:process.arch,glibc:'2.31',node:process.version,npm,
  ...
  packageReport:output.match(/Results: (.+)/)[1],parallelReport:parallelPath,checks,
  cliChecks:read(install.checks['installed-cli-regression'].stdout.match(/Results: (.+)/)[1]),
  parallelChecks:read(parallelPath)};
...
const result = {inputs,tarballSha256:hash(readFileSync(packed.tarball)),
...
console.log(`Debian 11 installed-package proof passed. Results: ${join(work,'result.json')}`);
